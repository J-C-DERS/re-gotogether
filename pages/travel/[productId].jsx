...
const ProductId = ({ post }) => {
  ...
  useEffect(() => {
    ...
    let recentCopy = localStorage.getItem("recent");
    recentCopy = JSON.parse(recentCopy);
    recentCopy.unshift(post.productId);

    recentCopy = new Set(recentCopy);
    recentCopy = Array.from(recentCopy);
    if (recentCopy.length > 5) {
      recentCopy.pop();
    }
    localStorage.setItem("recent", JSON.stringify(recentCopy));
  }, []);
